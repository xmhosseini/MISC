...
def lr_schedule(epoch):
    ...
    if epoch > 80*180//200:
        lr *= 0.5e-3
    elif epoch > 80*160//200:
        lr *= 1e-3
    elif epoch > 80*120//200:
        lr *= 1e-2
    elif epoch > 80*80//200:
        lr *= 1e-1
...
def Mobilenet_bottom():
    ...
    x = my_conv2d(inputs=x, my_filter = getCustomTensor(1, 1, 512, 512, 33, 3), filters=512, kernel_size=(1,1), strides=(1,1), padding='same',  kernel_initializer = 'glorot_uniform')
    x = BatchNormalization()(x)
    x = my_conv2d(inputs=x, my_filter = getCustomTensor(1, 1, 512, 512, 33, 13), filters=512, kernel_size=(1,1), strides=(1,1), padding='same',  kernel_initializer = 'glorot_uniform')
    x = BatchNormalization()(x)
...
# smh: this rate seems to be degrated to 0.001 through the course of epochs
    x = my_conv2d(inputs=x, my_filter = getCustomTensor(1, 1, 512, 1000, 33, 2), filters=1000, kernel_size=(1,1), strides=(1,1), padding='same',  kernel_initializer = 'glorot_uniform')
    x = BatchNormalization()(x)
    x = my_conv2d(inputs=x, my_filter = getCustomTensor(1, 1, 1000, 1000, 33, 29), filters=1000, kernel_size=(1,1), strides=(1,1), padding='same', kernel_initializer = 'glorot_uniform')
...
if __name__ == '__main__':
    ...
    checkpoint = ModelCheckpoint(filepath='%s.hdf5' % 'model91_arb', verbose=1,
