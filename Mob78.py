...
def Mobilenet_bottom():
    ...
    x = my_conv2d(inputs=x, my_filter = getCustomTensor(1, 1, 512, 512, 64, 1), filters=512, kernel_size=(1,1), strides=(1,1), padding='same',  kernel_initializer = 'glorot_uniform')
    x = BatchNormalization()(x)
    x = my_conv2d(inputs=x, my_filter = getCustomTensor(1, 1, 512, 512, 64, 8), filters=512, kernel_size=(1,1), strides=(1,1), padding='same',  kernel_initializer = 'glorot_uniform')
    x = BatchNormalization()(x)
...
# smh: this rate seems to be degrated to 0.001 through the course of epochs
    x = my_conv2d(inputs=x, my_filter = getCustomTensor(1, 1, 512, 1000, 100, 1), filters=1000, kernel_size=(1,1), strides=(1,1), padding='same',  kernel_initializer = 'glorot_uniform')
    x = BatchNormalization()(x)
    x = my_conv2d(inputs=x, my_filter = getCustomTensor(1, 1, 1000, 1000, 100, 10), filters=1000, kernel_size=(1,1), strides=(1,1), padding='same', kernel_initializer = 'glorot_uniform')
